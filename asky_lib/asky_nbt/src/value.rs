...
use crate::{
    error::Result,
    io::{self, CompressionType},
    options::NbtOptions,
    ser,
};

/// French docs: https://fr.minecraft.wiki/w/Format_NBT
...
impl Value {
    ...
    /// see french docs: https://fr.minecraft.wiki/w/Format_NBT
    #[must_use]
    ...
    ///
    /// # Args
    ///   # `compression` - NBT compressions type
    ///   # `options` - NBT options
    ...
    pub fn to_bytes(
        ...
        let mut writer = io::encode(Vec::new(), compression)?;
        ser::to_writer_value_with_options(&mut writer, self, root_name, options)?;
        writer.finish()
    }
}
